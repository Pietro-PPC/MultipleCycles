...
corMat$C2 <- matrix(nrow=7, ncol=1)
varMatC2 <- matrix(nrow=9, ncol=1)

corMat$C2[1] = cor(bv(newCycleSelections), ebv(newCycleSelections))

popList$F1 = makeCross(newCycleSelections, crossPlan = cross, nProgeny = 5)
varMatC2[3,] = varG(popList$F1)
gvMatC2[3,] <- mean(gv(popList$F1))
...
varMatC2[4,] = varG(popList$F2)
gvMatC2[4,] <- mean(gv(popList$F2))
...
corMat$C2[2] = cor(bv(popList$F2), ebv(popList$F2))
## Maybe can use setEbvRf
...
varMatC2[5,] = varG(popList$F3)
gvMatC2[5,] <- mean(gv(popList$F3))

setAllelesMat(allelesMat, "F3", popList)

#set EBV using RF model
setEbvRf("C2", popList, EBV, "F3", corMat, 3)
...
varMatC2[6,] = varG(popList$F4)
gvMatC2[6,] <- mean(gv(popList$F4))

setAllelesMat(allelesMat, "F4", popList)

##set EBV using BLUP model##
setEbvRf("C2", popList, EBV, "F4", corMat, 4)
...
varMatC2[7,]= varG(popList$F5)
gvMatC2[7,] <- mean(gv(popList$F5))
...
##set EBV using BLUP model##
setEbvRf("C2", popList, EBV, "F5", corMat, 5)
...
varMatC2[8,] = varG(popList$PYT)
gvMatC2[8,] <- mean(gv(popList$PYT))

setAllelesMat(allelesMat, "PYT", popList)

##set EBV using BLUP model##
setEbvRf("C2", popList, EBV, "PYT", corMat, 6)
...
varMatC2[9,] = varG(popList$AYT)
gvMatC2[9,] <- mean(gv(popList$AYT))

setAllelesMat(allelesMat, "AYT", popList)

##set EBV using BLUP model##
setEbvRf("C2", popList, EBV, "AYT", corMat, 7)
...
allelesMat$C2 <- rbind(allelesMat$NP, allelesMat$F1, allelesMat$F2, allelesMat$F3, allelesMat$F4, allelesMat$F5, allelesMat$PYT, allelesMat$AYT, allelesMat$Var)
...
bv_ebvC2 <- rbind(bvebv$a, bvebv$b, bvebv$c, bvebv$d, bvebv$e, bvebv$f, bvebv$g)
